...
from .xrefs import CallerInfo, StringRef
...
class FrameInfo:
    ...
    frame_id: int  # Frame structure ID
    local_size: int  # Size of local variables area
    saved_regs_size: int  # Size of saved registers area
    arg_size: int  # Size of function arguments
    fp_delta: int  # Frame pointer delta
...
class RegisterVar:
    ...
    start_ea: ea_t  # Start address where variable is valid
    end_ea: ea_t  # End address where variable is valid
    register: str  # Register name (e.g., 'eax', 'rdi')
...
class FunctionChunk:
    ...
    end_ea: ea_t
    is_main: bool
    owner_func: Optional[func_t] = None
...
class Functions(DatabaseEntity):
    ...
    def get_basic_blocks(self, func: func_t) -> Optional[FlowChart]:
        """
    ...
    def get_callers_info(self, func: func_t) -> List['CallerInfo']:
        """
        Gets detailed information about all callers of this function.

        This method provides richer information than get_callers(), returning
        CallerInfo objects that include call site addresses and caller names.

        Args:
            func: The function instance.

        Returns:
            List of CallerInfo objects with detailed caller information.
        """
        from .xrefs import CallerInfo
        return self.database.xrefs.get_function_callers(func.start_ea)

    def get_string_refs(self, func: func_t) -> List['StringRef']:
        """
        Gets all string references within this function.

        This method returns detailed information about each string reference,
        including the instruction address, string address, string value, and
        ...
        """
        from .xrefs import StringRef
        return self.database.xrefs.get_string_refs_in_function(func.start_ea)
    ...
    def get_frame_info(self, func: func_t) -> Optional[FrameInfo]:
        """
        Get function frame (stack) information.

        Args:
            func: Function object (must be entry chunk)

        Returns:
            FrameInfo object with stack frame details, or None if not entry chunk
        """
    ...
    def get_register_variables(self, func: func_t) -> List[RegisterVar]:
        """
        Get renamed register variables in function.

        Args:
            func: Function object (must be entry chunk)

        Returns:
            List of RegisterVar objects describing register variables
        """
        if not ida_funcs.is_func_entry(func):
            return []

        regvars = []
        for rv in func.regvars:
            regvars.append(
                RegisterVar(
                    name=rv.user,
                    start_ea=rv.start_ea,
                    end_ea=rv.end_ea,
                    register=rv.canon
                )
            )
    ...
    def get_register_arguments(self, func: func_t) -> List[RegisterVar]:
        """
        Get function register arguments.

        Args:
            func: Function object (must be entry chunk)

        Returns:
            List of RegisterVar objects for register-based arguments
        """
        if not ida_funcs.is_func_entry(func):
            return []

        regargs = []
        for ra in func.regargs:
            regargs.append(
                RegisterVar(
                    name=ra.name or 'arg',
    ...
    def get_tail_info(self, chunk: func_t) -> Optional[TailInfo]:
        ...
        return TailInfo(
            owner_ea=chunk.owner,
            owner_name=owner_name
        )

    def get_heads(self, func: func_t) -> Iterator[ea_t]:
        """
        Iterate over all instruction/data heads in the function.

        This method provides a way to iterate through all defined heads (instruction
        and data item start addresses) within a function's boundaries.

        Args:
            func: The function object

        Yields:
            Addresses of all heads within the function

        Example:
            >>> func = db.functions.get_at(0x401000)
            >>> for head in db.functions.get_heads(func):
            ...     print(f"Head at: 0x{head:x}")
        """
        ea = func.start_ea
    ...
    def get_addresses(self, func: func_t) -> Iterator[ea_t]:
        """
        Iterate over all addresses in the function (including undefined bytes).

        This method provides address-by-address iteration through the entire
        function range, including undefined bytes between heads.

        Args:
            func: The function object

        Yields:
            All addresses from start_ea to end_ea (exclusive)

        Example:
            >>> func = db.functions.get_at(0x401000)
            >>> for addr in db.functions.get_addresses(func):
            ...     flags = ida_bytes.get_flags(addr)
            ...     print(f"0x{addr:x}: flags={flags}")
        """
    ...
    def get_items(self, func: func_t) -> Iterator[ea_t]:
        """
        Iterate through all instruction addresses in the function.

        Args:
            func: The function to iterate through.

        Yields:
            Effective addresses of instructions in the function.
        """
        for head in ida_funcs.func_item_iterator_t(func):
            yield head

    def get_head_items(self, func: func_t) -> Iterator[ea_t]:
        """
        Iterate through head (non-tail) instruction addresses in the function.

        Args:
            func: The function to iterate through.

        Yields:
            Effective addresses of head instructions only.
        """
        for head in ida_funcs.func_item_iterator_t(func):
            if not self.database.bytes.is_tail_at(head):
                yield head

    def get_all_addresses(self, func: func_t) -> Iterator[ea_t]:
        """
        Iterate through all addresses in the function, including tails.

        Args:
            func: The function to iterate through.

        Yields:
            All effective addresses in the function range.
        """
        for ea in range(func.start_ea, func.end_ea):
            yield ea
