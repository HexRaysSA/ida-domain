...
1. Registered your IDA installation so the `idapro` module can find it. If IDA was installed via HCLI this is already done; otherwise run the `py-activate-idalib.py` script that ships with IDA once. See [Getting Started](getting_started.md) for details.
