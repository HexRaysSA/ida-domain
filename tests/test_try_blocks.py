...
def db_readonly(try_blocks_test_setup):
    """
    Open database for read-only try blocks testing.
...
@pytest.fixture(scope='function')
def db_mutable(try_blocks_test_setup):
    """
    Open database for mutation try blocks testing (fresh per test).

    RATIONALE: Tests that modify try block data (add, remove) need isolated
...
def test_get_in_range_returns_empty_for_range_without_try_blocks(db_readonly):
    """
    ...
    """
    start_ea = db_readonly.minimum_ea
    end_ea = db_readonly.maximum_ea

    result = list(db_readonly.try_blocks.get_in_range(start_ea, end_ea))
...
def test_get_in_range_validates_start_address(db_readonly):
    """
    ...
    """
    invalid_start = db_readonly.maximum_ea + 0x10000
    valid_end = db_readonly.maximum_ea

    with pytest.raises(InvalidEAError):
        list(db_readonly.try_blocks.get_in_range(invalid_start, valid_end))


def test_get_in_range_validates_end_address(db_readonly):
    """
    ...
    """
    valid_start = db_readonly.minimum_ea
    invalid_end = db_readonly.maximum_ea + 0x10000

    with pytest.raises(InvalidEAError):
        list(db_readonly.try_blocks.get_in_range(valid_start, invalid_end))


def test_get_in_range_rejects_reversed_range(db_readonly):
    """
    ...
    start_ea = db_readonly.minimum_ea + 0x10
    end_ea = db_readonly.minimum_ea  # end < start

    with pytest.raises(InvalidParameterError):
        list(db_readonly.try_blocks.get_in_range(start_ea, end_ea))
...
def test_get_at_returns_none_for_address_without_try_block(db_readonly):
    """
...
def test_get_at_validates_address(db_readonly):
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.get_at(invalid_ea)


def test_get_at_returns_none_for_address_outside_function(db_readonly):
    """
...
def test_is_in_try_block_returns_false_for_normal_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_in_try_block(ea)

    assert isinstance(result, bool), 'is_in_try_block should return boolean'


def test_is_in_try_block_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.is_in_try_block(invalid_ea)


def test_is_in_try_block_with_kind_filter_cpp(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_in_try_block(ea, TryBlockKind.CPP)

    assert isinstance(result, bool), 'is_in_try_block with filter should return boolean'


def test_is_in_try_block_with_kind_filter_seh(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_in_try_block(ea, TryBlockKind.SEH)

    assert isinstance(result, bool), 'is_in_try_block with SEH filter should return boolean'


def test_is_catch_start_returns_false_for_normal_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_catch_start(ea)

    assert isinstance(result, bool), 'is_catch_start should return boolean'


def test_is_catch_start_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.is_catch_start(invalid_ea)


def test_is_seh_handler_start_returns_false_for_normal_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_seh_handler_start(ea)

    assert isinstance(result, bool), 'is_seh_handler_start should return boolean'


def test_is_seh_handler_start_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.is_seh_handler_start(invalid_ea)


def test_is_seh_filter_start_returns_false_for_normal_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.is_seh_filter_start(ea)

    assert isinstance(result, bool), 'is_seh_filter_start should return boolean'


def test_is_seh_filter_start_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.is_seh_filter_start(invalid_ea)


def test_find_seh_region_returns_none_for_non_seh_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.find_seh_region(ea)
...
def test_find_seh_region_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.find_seh_region(invalid_ea)


def test_has_fallthrough_from_unwind_returns_false_for_normal_code(db_readonly):
    """
    ...
    result = db_readonly.try_blocks.has_fallthrough_from_unwind(ea)

    assert isinstance(result, bool), 'has_fallthrough_from_unwind should return boolean'


def test_has_fallthrough_from_unwind_validates_address(db_readonly):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_readonly.try_blocks.has_fallthrough_from_unwind(invalid_ea)
...
def test_add_try_block_validates_addresses(db_mutable):
    """
    ...
    with pytest.raises(InvalidEAError):
        db_mutable.try_blocks.add(try_block)


def test_add_try_block_rejects_empty_try_block(db_mutable):
    """
...
def test_add_try_block_rejects_cpp_without_catches(db_mutable):
    """
    ...
    end_ea = min(start_ea + 0x100, db_mutable.maximum_ea)
...
def test_remove_in_range_validates_start_address(db_mutable):
    ...
    """
    invalid_start = db_mutable.maximum_ea + 0x10000
...
def test_remove_in_range_validates_end_address(db_mutable):
    ...
    """
    valid_start = db_mutable.minimum_ea
...
def test_remove_in_range_rejects_reversed_range(db_mutable):
    ...
    """
    # Use valid addresses but in reversed order
    start_ea = db_mutable.minimum_ea + 0x10
