...
- **47 commits** with **+24,750 lines** of new code
- **10 new entity modules** added
- **150+ new methods** across all entities
- **16 new test files** with comprehensive coverage
- **218 mypy strict errors fixed** (218 → 0)

---

## New Entities

### Analysis (`ida_domain/analysis.py`)

Complete auto-analysis control and management.
...
### Imports (`ida_domain/imports.py`)

Import table enumeration and module operations.
...
### Search (`ida_domain/search.py`)

17 search methods for finding patterns in the database.
...
### StackFrames (`ida_domain/stack_frames.py`)
...
### Switches (`ida_domain/switches.py`)

Switch statement analysis and manipulation.
...
### TryBlocks (`ida_domain/try_blocks.py`)

Exception handling block analysis.

**CatchHandler Properties:**
- `is_catch_all()` - Check if catch-all handler
- `is_cleanup()` - Check if cleanup handler
- `start_ea()` / `end_ea()` - Handler address range
...
- `start_ea()` / `end_ea()` - Try block address range
...
### Exporter (`ida_domain/exporter.py`)
...
### Problems (`ida_domain/problems.py`)
...
### Fixups (`ida_domain/fixups.py`)

Relocation and fixup operations.
...
### Decompiler (`ida_domain/decompiler.py`)

Basic decompilation support.
...
### Instructions (`ida_domain/instructions.py`)
...
- `format_offset_expression(ea, operand_index, ...)` - Format offset expr
- `calculate_offset_base(ea, operand_index)` - Calculate offset base

---

### Types (`ida_domain/types.py`)
...
### Segments (`ida_domain/segments.py`)
...
### Functions (`ida_domain/functions.py`)
...
### Comments (`ida_domain/comments.py`)
...
### Database (`ida_domain/database.py`)
